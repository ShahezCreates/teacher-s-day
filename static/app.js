...
const fortunes = ['Today, your kindness will travel farther than you expect.', 'A small pause can make room for a brilliant idea.', 'Someone is carrying confidence because you gave it to them.', 'Your patience is a superpower — even when Wi-Fi is not.', 'The best lessons often become someone’s favourite memory.'];
$('#fortune').onclick = () => { $('#fortuneText').textContent = fortunes[Math.floor(Math.random() * fortunes.length)]; $('#fortuneText').classList.remove('pop'); void $('#fortuneText').offsetWidth; $('#fortuneText').classList.add('pop'); celebrate(); };
const compliments = ['You make a difference in ways you may never fully see.', 'Your students are lucky to have someone who believes in them.', 'Today’s reminder: you are doing better than you think.', 'You turn questions into confidence — that is real magic.', 'Your kindness is part of someone’s favourite school memory.'];
$('#compliment').onclick = () => { const text = $('#complimentText'); text.textContent = compliments[Math.floor(Math.random() * compliments.length)]; $('#compliment').classList.add('opened'); text.classList.remove('pop'); void text.offsetWidth; text.classList.add('pop'); celebrate(); };
